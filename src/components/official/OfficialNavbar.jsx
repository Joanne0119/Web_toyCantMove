...
const ThemeToggle = ({ className = '' }) => {
  ...
  const toggleTheme = () => {
    const newTheme = isDark ? 'bumblebee' : 'halloween';
    document.documentElement.setAttribute('data-theme', newTheme);
    setIsDark(!isDark);
  };

  return (
    <button onClick={toggleTheme} className={`btn btn-ghost btn-sm btn-circle cursor-pointer ${className}`} title={isDark ? '切換亮色' : '切換暗色'}>
      {isDark ? (
            ...
            <div className="hidden md:flex items-center gap-1">
              {navLinks.map(({ id, label }) => (
                <button
                  ...
                  className={`official-nav-link px-3 py-2 rounded-lg text-sm font-medium transition-all duration-200 cursor-pointer ${
                    activeSection === id
                      ? 'nav-active bg-primary/20'
              ...
              <button
                ...
                className="btn btn-primary btn-sm ml-2 cursor-pointer"
              >
                開始遊戲
              </button>
            </div>
